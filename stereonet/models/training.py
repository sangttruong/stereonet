...
def train(model, loader, optimizer, loss, stdzer, device, scheduler=None, task=None):
    model.train()
    ...
    for data in tqdm(loader, total=len(loader)):
        ...
        if not scheduler is None:
          scheduler.step()
        loss_all += loss(stdzer(out, rev=True), data.y).item()
...
def test(model, loader, loss, stdzer, device, task, viz_dir=None, viz_ids=None):
    ...
    if task == 'regression':
        return ys, preds, math.sqrt(error / len(loader.dataset)), None, None  # rmse
    elif task == 'classification':
        auc = roc_auc_score(ys, preds)
        return ys, preds, error / len(loader.dataset), correct / len(loader.dataset), auc
